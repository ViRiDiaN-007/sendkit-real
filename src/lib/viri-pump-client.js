...
class PumpChatClient extends events_1.EventEmitter {
        ...
        switch (messageType) {
            ...
            case "2": // Ping from server - Keep-alive mechanism
                ...
                try {
                    ...
                    console.error("🏓 [ERROR] Failed to send pong:", error);
                }
                break;
    ...
    handleConnect(data) {
        ...
        if (connectData.pingInterval) {
            // this.startPing(connectData.pingInterval); // Disabled - we only respond to server pings
        }
    ...
    send(data) {
        if (this.connection && this.isConnected) {
            ...
            console.error("Cannot send data: not connected. Connection:", !!this.connection, "isConnected:", this.isConnected);
        }
    }
    /**
     ...
     */
    sendPong() {
        console.log("🏓 [DEBUG] Sent pong - responding to server ping");
        console.log("🏓 [DEBUG] Connection state before pong:", this.isConnected);
        console.log("🏓 [DEBUG] Connection object exists:", !!this.connection);
        
        try {
            // Try sending pong with proper socket.io format
            this.send("3"); // "3" is the pong message in socket.io
            console.log("🏓 [DEBUG] Pong message sent: '3'");
            console.log("🏓 [DEBUG] Connection state after pong:", this.isConnected);
        } catch (error) {
            console.error("🏓 [ERROR] Failed to send pong:", error);
        }
    }
    ...
    /**
     * Generates a proper WebSocket key for the handshake.
